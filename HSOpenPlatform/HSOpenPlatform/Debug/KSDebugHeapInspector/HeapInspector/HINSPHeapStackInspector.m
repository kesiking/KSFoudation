...
+ (void)updateRegisteredClasses
{
    ...
    for (unsigned int i = 0; i < count; i++) {
        ...
        if (className && ![self _HINSPHeapClassNameIsKnownUnsafeClass:(__bridge NSString *)className] && [(__bridge NSString *)className characterAtIndex:0] != '_') {
...
+(BOOL)_HINSPHeapClassNameIsKnownUnsafeClass:(NSString*)className {
    if (className == nil) {
        return NO;
    }
