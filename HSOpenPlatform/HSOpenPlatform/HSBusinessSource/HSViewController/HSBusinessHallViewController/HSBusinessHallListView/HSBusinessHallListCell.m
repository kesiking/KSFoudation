...
#import "HSBusinessHallListCell.h"
...
@interface HSBusinessHallListCell ()
...
@implementation HSBusinessHallListCell
...
- (void)configWithModel:(AMapPOI *)poi {
    ...
    self.addressLabel.text = [NSString stringWithFormat:@"%@%@",poi.district,poi.address];
    self.phoneLabel.text = poi.tel;
    self.distanceLabel.text = [NSString stringWithFormat:@"距离：%@",[self transformDistance:poi.distance]];
}
