...
- (void)reCenterMap{
    ...
    AMapPOI *poi = self.poiList[self.selectedIndex];
...
- (MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id<MAAnnotation>)annotation
{
    if ([annotation isKindOfClass:[MAPointAnnotation class]])
    {
        ...
        HSBusinessHallAnnotationView *annotationView = (HSBusinessHallAnnotationView *)[mapView dequeueReusableAnnotationViewWithIdentifier:reuseIndetifier];
        if (annotationView == nil)
        {
            annotationView = [[HSBusinessHallAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:reuseIndetifier];
        }
        ...
        annotationView.poi = self.poiList[index];
        WEAKSELF
        annotationView.annotationSelectedBlock = ^(){
            ...
            AMapPOI *poi = strongSelf.poiList[index];
            [strongSelf.mapView setCenterCoordinate:CLLocationCoordinate2DMake(poi.location.latitude, poi.location.longitude) animated:YES];
            strongSelf.viewController.title = poi.name;
        };
